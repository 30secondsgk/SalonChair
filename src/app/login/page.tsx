...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    if (auth && !isSigningIn) {
      try {
        setIsSigningIn(true);
        // We now await the login process
      ...
      <Card className="w-full max-w-md rounded-3xl border-none shadow-xl overflow-hidden">
        <CardHeader className="bg-primary text-white text-center pb-8 pt-10">
          ...
          <CardDescription className="text-primary-foreground/80">
            Sign in with your Google account to book your next session
          </CardDescription>
        </CardHeader>
        <CardContent className="p-8 pt-10">
          <Button 
            ...
            className="w-full py-8 rounded-2xl shadow-lg shadow-primary/20 text-lg font-bold flex items-center justify-center gap-3 bg-white text-foreground hover:bg-muted border border-border disabled:opacity-70"
          >
            {isSigningIn ? (
